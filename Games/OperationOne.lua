...
local function createEsp(playerName)
    ...
    cache[playerName] = esp
end

local function isPlayerBehindWall(playerName)
...
local function cleanupAllEsp()
    for playerName, esp in pairs(cache) do
...
local function updateEsp()
    ...
    for playerName, esp in pairs(cache) do
        ...
        if character then
            ...
            local isBehindWall = ESP_SETTINGS.WallCheck and isPlayerBehindWall(playerName)
            ...
            if rootPart and head and humanoid and shouldShow then
                ...
                if onScreen then
                    ...
                    if ESP_SETTINGS.ShowName then
                        ...
                        esp.name.Text = string.lower(playerName)
            ...
            -- Character doesn't exist, hide everything
            esp.box.Visible = false
            esp.boxOutline.Visible = false
            esp.name.Visible = false
            esp.healthOutline.Visible = false
...
-- Function to scan workspace for player characters
local function scanForPlayers()
    for _, child in ipairs(workspace:GetChildren()) do
        if child:FindFirstChild("Humanoid") and child:FindFirstChild("HumanoidRootPart") and child:FindFirstChild("Head") then
            if not cache[child.Name] then
                createEsp(child.Name)
            end
        end
    end
end
...
-- Periodically scan for new players
while true do
